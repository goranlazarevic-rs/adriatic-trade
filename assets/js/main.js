const PRODUCTS={
 '10086':{sku:'10086',slug:'cvet-soli-100g',name:'Cvet soli - natron kesa 100 g',price:603.67,img:'/assets/img/products/cvet-soli-100g.png'},
 '10012':{sku:'10012',slug:'cvet-soli-125g',name:'Cvet soli - kutija sa plutanim poklopcem 125 g',price:862.73,img:'/assets/img/products/cvet-soli-125g.png'},
 '10306':{sku:'10306',slug:'nerafinisana-sitna-600g',name:'BIO nerafinisana sitna morska so 600 g ZIP',price:322.61,img:'/assets/img/products/nerafinisana-sitna-600g.png'},
 '10307':{sku:'10307',slug:'nerafinisana-krupna-600g',name:'BIO nerafinisana krupna morska so 600 g ZIP',price:322.61,img:'/assets/img/products/nerafinisana-krupna-600g.png'},
 '10240':{sku:'10240',slug:'nerafinisana-sitna-500g',name:'BIO nerafinisana sitna morska so 500 g - kutija sa poklopcem',price:559.19,img:'/assets/img/products/nerafinisana-sitna-500g.png'}
};
const FREE_SHIPPING=2500;
const SHIPPING_FEE=300;
const fmt=n=>new Intl.NumberFormat('sr-RS',{minimumFractionDigits:2,maximumFractionDigits:2}).format(n)+' RSD';
...
function renderCheckout(){
 ...
 const t=cartTotal(c),free=t>=FREE_SHIPPING,shipping=free?0:SHIPPING_FEE,total=t+shipping,orderId=makeOrderId();
 const lines=entries.map(([sku,q])=>`${PRODUCTS[sku].name} × ${q} = ${fmt(PRODUCTS[sku].price*q)}`);
 ...
 function syncDelivery(){const method=form.querySelector('[name="Nacin isporuke"]:checked')?.value||'Dostava na adresu';const parcel=method.startsWith('Paketomat');if(address){address.required=!parcel;address.placeholder=parcel?'Nije obavezno za paketomat':'Ulica i broj'}if(addressLabel)addressLabel.textContent=parcel?'Adresa i broj (opciono)':'Adresa i broj *';if(parcelNote)parcelNote.classList.toggle('hidden',!parcel)}
