const PRODUCTS={
 ...
 'PKT-PROBA':{sku:'PKT-PROBA',name:'Paket za probu',price:1149,img:'/assets/img/packages/paket-za-probu.jpg',bundle:'Cvet soli 100 g + sitna 600 g + krupna 600 g'},
 'PKT-GURMAN':{sku:'PKT-GURMAN',name:'Gurmanski paket',price:1849,img:'/assets/img/packages/gurmanski-paket.jpg',bundle:'Cvet soli 125 g + Cvet soli 100 g + sitna 500 g'},
 'PKT-KOMPLET':{sku:'PKT-KOMPLET',name:'Komplet Solana Nin',price:2399,img:'/assets/img/packages/komplet-solana-nin.jpg',bundle:'Svih 5 proizvoda',freeShipping:true}
};
...
const hasFreeShippingBundle=c=>Object.entries(c).some(([sku,q])=>q>0&&PRODUCTS[sku]?.freeShipping);
const shippingFor=c=>{const t=cartTotal(c);const free=t>=FREE_SHIPPING||hasFreeShippingBundle(c);return {t,free,shipping:free?0:SHIPPING_FEE,total:t+(free?0:SHIPPING_FEE)}};
...
function renderCart(){const root=document.querySelector('[data-cart-list]');if(!root)return;const c=getCart();const entries=Object.entries(c).filter(([s,q])=>PRODUCTS[s]&&q>0);if(!entries.length){root.innerHTML='<div class="card"><h2 class="h3">Korpa je prazna.</h2><p class="muted">Dodajte proizvod iz našeg asortimana.</p><a class="btn btn-primary" href="/proizvodi.html">Pogledaj proizvode</a></div>';document.querySelector('[data-cart-summary]').innerHTML='';return}root.innerHTML=entries.map(([sku,q])=>{const p=PRODUCTS[sku];return `<div class="cart-item"><img src="${p.img}" alt=""><div><b>${p.name}</b><div class="muted">${fmt(p.price)} / kom${p.bundle?` • ${p.bundle}`:''}</div><div class="qty" style="margin-top:8px"><button data-minus="${sku}" aria-label="Smanji količinu">−</button><strong>${q}</strong><button data-plus="${sku}" aria-label="Povećaj količinu">+</button><button class="btn btn-danger btn-small" data-remove="${sku}">Ukloni</button></div></div><strong>${fmt(p.price*q)}</strong></div>`}).join('');root.querySelectorAll('[data-minus]').forEach(b=>b.onclick=()=>{const c=getCart();c[b.dataset.minus]-=1;if(c[b.dataset.minus]<=0)delete c[b.dataset.minus];saveCart(c);renderCart()});root.querySelectorAll('[data-plus]').forEach(b=>b.onclick=()=>{const c=getCart();c[b.dataset.plus]+=1;saveCart(c);renderCart()});root.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{const c=getCart();delete c[b.dataset.remove];saveCart(c);renderCart()});renderSummary()}
function renderSummary(){const box=document.querySelector('[data-cart-summary]');if(!box)return;const c=getCart(),{t,free,shipping,total}=shippingFor(c),remain=Math.max(0,FREE_SHIPPING-t),pct=Math.min(100,t/FREE_SHIPPING*100);box.innerHTML=`<div class="order-box"><h2 class="h3">Pregled porudžbine</h2><div class="order-line"><span>Vrednost robe</span><b>${fmt(t)}</b></div><div class="order-line"><span>Dostava</span><b>${free?'Besplatna':fmt(shipping)}</b></div><div class="free-progress"><span style="width:${pct}%"></span></div><p class="muted" style="font-size:.88rem">${free?(hasFreeShippingBundle(c)?'Besplatna dostava je uključena uz Komplet Solana Nin.':'Ostvarili ste besplatnu dostavu.'):'Još '+fmt(remain)+' do besplatne dostave.'}</p><div class="order-total"><span>Ukupno za plaćanje</span><span>${fmt(total)}</span></div><a class="btn btn-primary" style="width:100%;margin-top:18px" href="/checkout.html">Nastavi na poručivanje</a><p class="muted" style="font-size:.78rem;margin-bottom:0">Plaćanje pouzećem. Dostava je 300 RSD za porudžbine ispod 2.500 RSD, a besplatna od 2.500 RSD.</p></div>`}
...
function renderCheckout(){
 ...
 const {t,free,shipping,total}=shippingFor(c),orderId=makeOrderId();
 const lines=entries.map(([sku,q])=>{const p=PRODUCTS[sku];return `${p.name}${p.bundle?' ['+p.bundle+']':''} × ${q} = ${fmt(p.price*q)}`});
 root.innerHTML=`<div class="checkout-summary"><h3 class="h3">Vaša porudžbina</h3><p class="muted">Broj: <b>${orderId}</b></p>${entries.map(([sku,q])=>`<div class="order-line"><span>${PRODUCTS[sku].name}${PRODUCTS[sku].bundle?` <small class="muted">(${PRODUCTS[sku].bundle})</small>`:''} × ${q}</span><b>${fmt(PRODUCTS[sku].price*q)}</b></div>`).join('')}<div class="order-line"><span>Dostava</span><b>${free?'0,00 RSD - besplatna':fmt(shipping)}</b></div><div class="order-total"><span>Ukupno za plaćanje</span><span>${fmt(total)}</span></div></div>`;
