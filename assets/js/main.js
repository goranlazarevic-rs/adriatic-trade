const PRODUCTS={
 ...
 'PKT-GURMAN':{sku:'PKT-GURMAN',name:'Gurmanski paket',price:1849,img:'/assets/img/story-premium/fleur-rake.webp',detail:'Cvet soli 125 g + Cvet soli 100 g + sitna 500 g kutija'},
 'PKT-KOMPLET':{sku:'PKT-KOMPLET',name:'Komplet Solana Nin',price:2399,img:'/assets/img/story-premium/harvest-workers-wide.webp',detail:'Svih 5 proizvoda iz online asortimana',freeShipping:true}
};
...
const cartHasFreeShipping=c=>Object.entries(c).some(([sku,q])=>q>0&&PRODUCTS[sku]?.freeShipping);
const getsFreeShipping=c=>cartTotal(c)>=FREE_SHIPPING||cartHasFreeShipping(c);
...
function renderSummary(){const box=document.querySelector('[data-cart-summary]');if(!box)return;const c=getCart(),t=cartTotal(c),free=getsFreeShipping(c),shipping=free?0:SHIPPING_FEE,total=t+shipping,remain=Math.max(0,FREE_SHIPPING-t),pct=Math.min(100,t/FREE_SHIPPING*100);box.innerHTML=`<div class="order-box"><h2 class="h3">Pregled porudžbine</h2><div class="order-line"><span>Vrednost robe</span><b>${fmt(t)}</b></div><div class="order-line"><span>Dostava</span><b>${free?'Besplatna':fmt(shipping)}</b></div><div class="free-progress"><span style="width:${pct}%"></span></div><p class="muted" style="font-size:.88rem">${free?(cartHasFreeShipping(c)&&t<FREE_SHIPPING?'Komplet Solana Nin uključuje besplatnu dostavu.':'Ostvarili ste besplatnu dostavu.'):'Još '+fmt(remain)+' do besplatne dostave.'}</p><div class="order-total"><span>Ukupno za plaćanje</span><span>${fmt(total)}</span></div><a class="btn btn-primary" style="width:100%;margin-top:18px" href="/checkout.html">Nastavi na poručivanje</a><p class="muted" style="font-size:.78rem;margin-bottom:0">Plaćanje pouzećem. Dostava je 300 RSD za porudžbine ispod 2.500 RSD, a besplatna od 2.500 RSD.</p></div>`}
